//
//  OdinUIShareSheetViewController.m
//  OdinShareSDKUI
//
...
#import "OdinUICollectionViewSimpleCell.h"
//#import "OdinUICollectionViewSimpleCell.h"
#import "OdinUICollectionViewLayout.h"
#import "OdinUIPlatformItem.h"
#import "OdinColor.h"
#import "OdinDevice.h"
#import "OdinUIConst.h"
#import <OdinShareSDK/OdinSocialManager.h>


#define kScreenWidth ([UIScreen mainScreen].bounds.size.width)
#define kScreenHeight ([UIScreen mainScreen].bounds.size.height)
#define isiPhoneXDevice NO

#define OdinUILocalized(key) NSLocalizedStringWithDefaultValue((key), @"ShareSDKUI_Localizable", [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"ShareSDKUI" ofType:@"bundle"]], @"Cancel", nil)
...
@interface OdinUIShareSheetViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
...
static NSString *const kSSUICollectionViewCellReuseIdentifier = @"OdinUICollectionViewCellReuseIdentifier";

@implementation OdinUIShareSheetViewController
...
- (void)viewDidLoad
{
    ...
    self.view.backgroundColor = [OdinColor colorWithARGB:0x4c000000];
...
- (void)_configUI
{
    ...
    if (self.configuration.style == OdinUIActionSheetStyleSystem)
    {
    ...
    if (self.configuration.style == OdinUIActionSheetStyleSimple)
    {
        platformsCollectionView.layer.borderWidth = 0.5;
        platformsCollectionView.layer.borderColor = [OdinColor colorWithRGB:0xF5F5F5].CGColor;
    }
    ...
    Class cellClass =self.configuration.style == OdinUIActionSheetStyleSystem ? OdinUICollectionViewCell.class:OdinUICollectionViewSimpleCell.class;
    ...
    [cancelButton setTitle:OdinUILocalized(@"Cancel") forState:UIControlStateNormal];
    ...
    cancelButton.hidden = (self.configuration.style==OdinUIActionSheetStyleSimple || self.configuration.isCancelButtonHidden);
    _cancelButton = cancelButton;
}

- (void)_setLayoutConstraints
{
    BOOL isSystemStyle = self.configuration.style == OdinUIActionSheetStyleSystem;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 也有可能是 OdinUIPlatformItem 类型 （用户自定义的）
    if ([platform isKindOfClass:NSNumber.class])
    {
        platform = [OdinUIPlatformItem itemWithPlatformType:[platform integerValue]];
    }
...
//        [weakSelf.delegate shareSheet:weakSelf didCancelShareWithParams:weakSelf.params];
    ...
    OdinUIPlatformItem *platformItem=self.platforms[indexPath.row];
    [[OdinSocialManager defaultManager] shareToPlatform:platformItem.platformType messageObject:self.shareObject currentViewController:self.currentVC completion:^(id result, NSError *error) {
        if (self.delegate&&[self.delegate respondsToSelector:@selector(shareSheet:didSelectPlatform:result:error:)]) {
            [self.delegate shareSheet:self didSelectPlatform:@(platformItem.platformType) result:result error:error];
        }
    }];
}
...
- (CGFloat)itemWidth
{
    ...
    {
        CGFloat sheetWidth = OdinDevice.isPad ? kShareSheetiPadWidth:kScreenWidth;
...
{
    CGFloat sheetWidth = OdinDevice.isPad ? kShareSheetiPadWidth:(kScreenWidth - 2 * self.menuSpace);
    NSInteger columns = self.totalColumns;
...
- (CGFloat)menuSpace
{
    ...
    {
        BOOL isSystemStyle = self.configuration.style == OdinUIActionSheetStyleSystem;
        return ((isSystemStyle && !OdinDevice.isPad) ? kMenuSpace:0.0);
...
- (OdinUICollectionViewLayout *)flowLayoutForMenu
{
    OdinUICollectionViewLayout *layout = [[OdinUICollectionViewLayout alloc] init];
